...
BOOST_AUTO_TEST_CASE(blockmanager_find_block_pos)
{
    ...
    BOOST_CHECK_EQUAL(blockman.WriteBlock(params->GenesisBlock(), 0).nPos, BLOCK_LEGACY_SERIALIZATION_HEADER_SIZE);
    // simulate what happens during reindex
    // simulate a well-formed genesis block being found at offset 8 in the blk00000.dat file
    // the block is found at offset 8 because there is an 8 byte serialization header
    // consisting of 4 magic bytes + 4 length bytes before each block in a well-formed blk file.
    const FlatFilePos pos{0, BLOCK_LEGACY_SERIALIZATION_HEADER_SIZE};
    ...
    BOOST_CHECK_EQUAL(actual.nPos, BLOCK_LEGACY_SERIALIZATION_HEADER_SIZE + ::GetSerializeSize(TX_WITH_WITNESS(params->GenesisBlock())) + BLOCK_LEGACY_SERIALIZATION_HEADER_SIZE);
}
...
BOOST_AUTO_TEST_CASE(blockmanager_flush_block_file)
{
    ...
    BOOST_CHECK_EQUAL(blockman.CalculateCurrentUsage(), (TEST_BLOCK_SIZE + BLOCK_LEGACY_SERIALIZATION_HEADER_SIZE) * 2);
